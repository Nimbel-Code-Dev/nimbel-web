export const es = {
  meta: {
    title: 'Nimbel CODE - Design Smarter, Not Harder',
    description: 'Tendrás la flexibilidad de un freelance y la seriedad de una consultora. Desarrollamos tu aplicación con calidad, personalización y a un coste razonable.',
  },
  navbar: {
    about: 'Sobre nosotros',
    services: 'Servicios',
    cta: 'AGENDA UNA CALL',
  },
  hero: {
    eyebrow: 'DESIGN SMARTER, NOT HARDER',
    headline: 'Más que el desarrollo de una aplicación.',
    description:
      'Nos encargamos de todo, desde la primera idea hasta que funciona y crece contigo. Tú solo cuéntanos qué necesitas.',
    cta: 'Hablemos de tu idea',
    imgAlt: 'Nimbel app preview',
  },
  clients: {
    headline: 'Confían en nosotros',
    ariaLabel: 'Carrusel de logos de clientes',
  },
  services: {
    headline: 'Desarrollo de aplicaciones a medida, Apps móvil y plataformas web',
    headlineHighlight1: 'Desarrollo',
    headlineHighlight2: 'plataformas',
    sectionLabel: 'Servicios',
    items: [
      {
        id: 'dev-infrastructure',
        title: 'Desarrollo de aplicaciones e infrasctructura',
        shortDesc: 'Construimos tu software desde cero y nos aseguramos de que funcione sin caídas, sea rápido y escale cuando tu negocio crezca.',
        fullDesc:
          'Construimos tu software desde cero y nos aseguramos de que funcione sin caídas, sea rápido y escale cuando tu negocio crezca.',
      },
      {
        id: 'ux-design',
        title: 'Diseño UX/UI',
        shortDesc: 'Que sea bonito está bien. Que tu equipo lo use sin formación y sin frustraciones es lo que importa.',
        fullDesc:
          'Que sea bonito está bien. Que tu equipo lo use sin formación y sin frustraciones es lo que importa.',
      },
      {
        id: 'sales-automation',
        title: 'Vende más sin trabajar el doble',
        shortDesc: 'Automatizamos tu proceso de ventas con IA: seguimientos, propuestas, captación. Para que ningún cliente potencial se pierda por falta de tiempo.',
        fullDesc:
          'Automatizamos tu proceso de ventas con IA: seguimientos, propuestas, captación. Para que ningún cliente potencial se pierda por falta de tiempo.',
      },
      {
        id: 'project-rescue',
        title: 'Rescate de proyectos tecnológicos',
        shortDesc: '¿Tu proyecto se quedó a medias, no funciona como esperabas o el proveedor anterior desapareció? Lo retomamos desde donde esté.',
        fullDesc:
          '¿Tu proyecto se quedó a medias, no funciona como esperabas o el proveedor anterior desapareció? Lo retomamos desde donde esté.',
      },
    ],
  },
  stats: {
    ariaLabel: 'Estadísticas de Nimbel Code',
    items: [
      { value: 15, label: 'AÑOS DE EXPERIENCIA' },
      { value: 50, label: 'PROYECTOS EXITOSOS' },
      { value: 25, label: 'PROYECTOS RESCATADOS' },
    ],
  },
  caseStudies: {
    headline: 'PROYECTOS DESTACADOS',
    ariaLabel: 'Carrusel de casos de éxito',
    items: [
      {
        id: 'famba',
        title: 'Famba, el mejor equipo para la salud del viajero internacional',
        titleHighlight: 'Famba,',
        description:
          'Desarrollo de aplicación móvil, consola de atención para doctores, consola de análisis de datos y estadísticas.',
        cta: 'conoce FAMBA',
        imgAlt: 'Captura del proyecto Famba',
      },
      {
        id: 'analysta',
        title: 'Analysta, analiza tu operativa en los mercados financieros',
        titleHighlight: 'Analysta,',
        description:
          'Analysta es una herramienta digital diseñada para simplificar el análisis de la operativa de un inversor en mercados financieros.',
        cta: 'conoce ANALYSTA',
        imgAlt: 'Captura del proyecto Analysta',
      },
      {
        id: 'innocard',
        title: 'Innocard, crea, personaliza y comparte toda tu identidad personal y corporativa en un solo gesto',
        titleHighlight: 'Innocard',
        description:
          'Plataforma que permite crear tarjetas de presentación digitales personalizadas.',
        cta: 'conoce INNOCARD',
        imgAlt: 'Captura del proyecto Innocard',
      },
      {
        id: 'newsup',
        title: 'NEWSUP crea, personaliza y comparte toda tu identidad',
        titleHighlight: 'NEWSUP',
        description:
          'Newsup es una plataforma digital de noticias, que permite tener una sala de prensa, enviar emails y compartir contenido multimedia HD.',
        cta: 'conoce NEWSUP',
        imgAlt: 'Captura del proyecto NEWSUP',
      },
    ],
  },
  testimonials: {
    headline: 'Lo que dicen nuestros clientes',
    ariaLabel: 'Carrusel de testimonios',
    starsAriaLabel: '5 de 5 estrellas',
    items: [
      {
        id: 'jose-munoz',
        name: 'Jose Muñoz',
        role: 'Jefe del Servicio de Salud Internacional',
        quote:
          'Famba llevaba años en desarrollo sin llegar a funcionar como necesitábamos. Cuando Nimbel Code se incorporó al proyecto, todo cambió. En poco tiempo lograron estabilizar la plataforma, mejorar la experiencia de usuario y convertir una idea estancada en una herramienta real y útil. Su enfoque claro, profesional y resolutivo fue justo lo que necesitábamos para sacar Famba adelante.',
        photoAlt: 'Foto de Jose Muñoz',
      },
      {
        id: 'pol-margaix',
        name: 'Pol Margaix',
        role: 'Fundador de Analysta',
        quote:
          'La colaboración con Nimbel ha sido fundamental para el desarrollo del proyecto. Destacaría la increíble eficiencia y su capacidad para ir siempre un paso por delante. No se limitan únicamente a ejecutar, sino que constantemente proponen soluciones innovadoras y mejoras que demuestran un profundo entendimiento de nuestras metas. Son un equipo resolutivo que no necesita supervisión constante, lo que nos ha permitido avanzar a un ritmo que no habíamos logrado antes. Su proactividad ha sido clave para desbloquear el verdadero potencial del proyecto.',
        photoAlt: 'Foto de Pol Margaix',
      },
      {
        id: 'gerard-gil',
        name: 'Gerard Gil',
        role: 'Fundador de NewsUp',
        quote:
          'Trabajar con Nimbel ha sido un punto de inflexión para Newsup. Supieron entender las necesidades tanto técnicas como editoriales del proyecto, y nos ayudaron a transformar nuestra plataforma en una herramienta mucho más robusta, escalable y fácil de gestionar. El equipo aportó no solo capacidad técnica, sino también visión de producto. Gracias a su trabajo, hoy podemos crecer y ofrecer una mejor experiencia a nuestros usuarios.',
        photoAlt: 'Foto de Gerard Gil',
      },
    ],
  },
  team: {
    headline: 'CONOCE A NUESTRO EQUIPO',
    ariaLabel: 'Equipo de Nimbel Code',
    linkedinAriaLabel: 'LinkedIn de',
    items: [
      {
        id: 'ruben-sahagun',
        name: 'Rubén Sahagún',
        role: 'CEO / CTO',
        photoAlt: 'Foto de Rubén Sahagún, CEO/CTO de Nimbel Code',
      },
      {
        id: 'giampiero-fermini',
        name: 'Giampiero Fermini',
        role: 'Full Stack Developer',
        photoAlt: 'Foto de Giampiero Fermini, Full Stack Developer de Nimbel Code',
      },
      {
        id: 'maribel-rodriguez',
        name: 'Maribel Rodríguez',
        role: 'UX/UI Designer',
        photoAlt: 'Foto de Maribel Rodríguez, UX/UI Designer de Nimbel Code',
      },
      {
        id: 'diana-guirado',
        name: 'Diana Guirado',
        role: 'CFO',
        photoAlt: 'Foto de Diana Guirado, CFO de Nimbel Code',
      },
      {
        id: 'clara-guirado',
        name: 'Clara Guirado',
        role: 'Operations Coordinator',
        photoAlt: 'Foto de Clara Guirado, Operations Coordinator de Nimbel Code',
      },
      {
        id: 'sol-lavisse',
        name: 'Sol Lavisse',
        role: 'Growth Manager',
        photoAlt: 'Foto de Sol Lavisse, Growth Manager de Nimbel Code',
      },
    ],
  },
  socialLinks: {
    linkedinLabel: 'LinkedIn',
    githubLabel: 'GitHub',
    twitterLabel: 'Twitter',
    visitOn: 'Visitar',
    on: 'en',
  },
  contact: {
    headline: '¿TIENES ALGÚN PROYECTO EN MENTE?',
    headlineHighlight: 'PROYECTO',
    description: 'Este puede ser el comienzo de algo importante. Envíanos un correo con las necesidades de tu proyecto y te responderemos en 1 o 2 días laborables (probablemente antes).',
    form: {
      name: 'Nombre',
      namePlaceholder: 'Tu nombre completo',
      email: 'Email',
      emailPlaceholder: 'tu@email.com',
      message: 'Cuéntanos sobre tu proyecto',
      messagePlaceholder: 'Cuéntanos sobre tu proyecto',
      submit: 'Enviar mensaje',
      sending: 'Enviando...',
    },
    contactOptions: [
      {
        type: 'email',
        label: 'CONTACTA',
        sublabel: 'Envíanos un email',
      },
      {
        type: 'call',
        label: 'AGENDA UNA CALL',
        sublabel: 'Agenda en nuestro calendly',
      },
      {
        type: 'linkedin',
        label: 'LiNKEDiN',
        sublabel: 'No somos mucho de redes sociales pero tenemos linkedin!',
      },
    ],
    errors: {
      required: 'Este campo es requerido',
      invalidEmail: 'El email no tiene un formato válido',
      minLength: 'Debe tener al menos {min} caracteres',
      maxLength: 'No puede superar los {max} caracteres',
    },
    success: {
      title: '¡Mensaje enviado!',
      message: 'Gracias por contactarnos. Te respondemos en menos de 24 horas.',
    },
  },
  footer: {
    description: 'Flexibilidad de freelance, seriedad de consultora.',
    nav: {
      title: 'Navegación',
      about: 'Sobre nosotros',
      services: 'Servicios',
      contact: 'Contacto',
    },
    social: {
      title: 'Seguinos',
    },
    copyright: '© 2026 Nimbel Code. Todos los derechos reservados.',
  },
} as const;
